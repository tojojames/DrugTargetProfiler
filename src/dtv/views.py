...
def search(request):
    # pdb.set_trace()
    return render(request, "home.html", {'nbar': 'search'})

def help(request):
    return render(request, "help.html", {'nbar': 'help'})

def autosuggest(request):
    term = ''.join([request.GET['term'].upper(),'%']) # join '%' with the parameter for the like sql statement
    cursor = connection.cursor()
    sql = "SELECT distinct term_name as autosuggest_result from drugtargetcommons.start_search  where (term_type = 'TARGET' or term_type = 'COMPOUND')  and term_name not like '%(comma)s' and term_name not like '%(braces)s' and term_name not like '%(mutant)s' and term_name like '%(my_like)s' LIMIT 40" % {'my_like': term,'comma':'%,%','braces':'%(%','mutant':'%Mutant%'}
    cursor.execute(sql)
    row = cursor.fetchall()
