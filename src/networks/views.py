...
def export_to_excel(request):
    ...
    #set column order
    df = df[['compound_id', 'compound_name','max_phase','target_id','gene_name','wild_type_mutated','mutation_info','family_name','ep_standardtype','ep_standardrelation','ep_standardvalue','ep_standardunits','assay_format','interaction_strength','dtc_molregno','dtc_tid','pubmed_id']]
    median_interaction_score = get_median_interaction_score(df,compounds)
    #get Clinical data
    c_data = ClinicalData.objects.filter(drug__in=compounds).defer("id").values()
    df_clinicaldata = pd.DataFrame(list(c_data))
    #reorder clinical data columns
    df_clinicaldata = df_clinicaldata[['study_id', 'study_title', 'study_type','drug','phase','adverse_effects', 'symptoms',  'enrollment', 'gender', 'max_age', 'mesh_term', 'min_age','start_date','dtc_molregno', 'pubmed_ids']]
    ...
    if not median_interaction_score.empty: median_interaction_score.to_excel(writer, sheet_name='Median-Interaction-Scores',index=True)
    ...
    if not get_ccle.empty: get_ccle.to_excel(writer, sheet_name='Gene expression CCLE',index=False)
    if not get_gdsc.empty: get_gdsc.to_excel(writer, sheet_name='Gene expression GDSC',index=False)
    if not get_gene_tech.empty: get_gene_tech.to_excel(writer, sheet_name='Gene expression Genetech',index=False)
    if not get_comp_ccle.empty: get_comp_ccle.to_excel(writer, sheet_name='Drug sensitivity CCLE',index=False)
    if not get_comp_gdsc.empty: get_comp_gdsc.to_excel(writer, sheet_name='Drug sensitivity GDSC1000',index=False)
    if not get_comp_gcsi.empty: get_comp_gcsi.to_excel(writer, sheet_name='Drug sensitivity gCSI',index=False)
    if not get_comp_grey.empty: get_comp_grey.to_excel(writer, sheet_name='Drug sensitivity GREY',index=False)
    if not get_comp_fimm.empty: get_comp_fimm.to_excel(writer, sheet_name='Drug sensitivity FIMM',index=False)
    if not get_comp_ctrp.empty: get_comp_ctrp.to_excel(writer, sheet_name='Drug sensitivity CTRPv2',index=False)
    if not get_comp_uhn.empty: get_comp_uhn.to_excel(writer, sheet_name='Drug sensitivity UHNBreast',index=False)
...
def get_median_interaction_score(data,compounds):

    genes = list(filter(None, data.gene_name.unique().tolist()))
    mutations = list(filter(None,data.mutation_info.unique().tolist()))
    columns = mutations + genes
    rows = list(filter(None,data.compound_name.unique().tolist()))
    df = pd.DataFrame(index=rows, columns=columns)
    for compound in compounds:
        compound_data = data[data['compound_name'] == compound]
        genes = list(filter(None,compound_data.gene_name.unique().tolist()))
        for gene in genes:
            gene_data = compound_data[(compound_data['gene_name'] == gene) & (compound_data['mutation_info'] == "")]
            value = gene_data['interaction_strength'].median()
            df.xs(compound)[gene] = value
            mutations = list(filter(None,compound_data[(compound_data['gene_name'] == gene) & (compound_data['mutation_info'] != "")].mutation_info.unique().tolist()))
            for mutation in mutations:
                mutation_data = compound_data[(compound_data['gene_name'] == gene) & (compound_data['mutation_info'] == mutation)]
                value = mutation_data['interaction_strength'].median()
                df.at[compound,mutation] = value
                df.set_value(compound,mutation, value)            
    return df            


def target_disease_info(genes):
    cursor = connection.cursor()
    genes = tuple(genes) if len(genes) > 1 else str(tuple(genes)).replace(',','')
    params = {'genes': genes}
    sql = "SELECT distinct  gene_name,  disease_name, score,  description,no_of_snps, source, disease_id,dtc_tid, pubmed_id from network_visuals.gene_disease_association where gene_name IN %(genes)s" % params
    cursor = connection.cursor()
    cursor.execute(sql)
    rows = dictfetchall(cursor)
    df = pd.DataFrame(rows)
    df = df[['gene_name','disease_name','score','description', 'no_of_snps' ,'source' ,'disease_id' ,'dtc_tid','pubmed_id']]
    return df
...
def getCcle(genes):
    ...
    df = df[['gene_name','cell_line','expression_val']]
    return df

def getGdsc(genes):
    df = pd.DataFrame()
    for g in genes:
        GdscQuerySet = Gdsc.objects.filter(gene_name=g).values('gene_name', 'cell_line','expression_val').order_by('-expression_val')[:10]
        if GdscQuerySet:
...
def getGeneTech(genes):
    ...
    df = pd.DataFrame()
    for g in genes:
        geneTechQuery = Genetech.objects.filter(gene_name=g).values('gene_name', 'cell_line','expression_val').order_by('-expression_val')[:10]
        if geneTechQuery:
...
def getCellLines(compounds,db):
    ...
    for comp in compounds:
        ...
        if rows:
            df = df.append(rows)
    if not df.empty:                   
        df = df[['drug_name','cell_name','dataset_name', 'dss1']]        
    return df        
